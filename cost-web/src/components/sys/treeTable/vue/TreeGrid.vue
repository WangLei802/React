<template >
  <el-table class="tree-table" ref="treeGrid"
    :data="data" stripe :height="tHeight"
    border
            id="crTable"
    style="width: 100%"
    :row-style="showTr" @selection-change="handleSelectionChange" size="small">
    <el-table-column type="selection" v-if="showCheckbox" :reserve-selection="true" prop='id' width="55" >
    </el-table-column>
      ...
      data: function() {
        ...
        if (me.treeStructure) {
          ...
          this.$nextTick(function() {
            this.data.map(item => {
              this.$refs.treeGrid.toggleRowSelection(item, item.isCurr === '1')
            })
          })
          return data
        }
