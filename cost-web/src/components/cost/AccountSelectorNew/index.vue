...
import {  saveOrUpdateIneffectiveCost, getQueryContractList, getContractDetail, getVisaList,
  getIneffectiveAlteration, getContractListByAlterationStatus, getIneffectiveCostCode} from '@/api/cost/ineffectiveCostModel'
...
export default {
  ...
  data: function () {
    return {
      rules: {
        accObjId: [{ message: '请选择成本分期', required: true, trigger: 'blur' }],
        projectId: [{ message: '请选择项目名称', required: true, trigger: 'blur' }]
      },
      ENUMTYPE: {
        'GROUP': 'GROUP',
        'COMPANY': 'COMPANY',
  ...
  methods: {
    ...
    getIneffectiveCostCode () {
      ...
      getIneffectiveCostCode(param)
        .then(response => {
          ...
          localStorage.setItem("applyNo", response.data);
          // debugger
          // this.emitValue(response.data)
        })
        .catch(error => {
          console.log(error)
    ...
    changeSelect (type, event) {
      ...
      if (event && this.totalData.allData) {
        ...
        localStorage.setItem("projectName", selectItem.text)
