...
<template>
  <div ref="designAlterationTable">
    ...
    >
      ...
      <!-- 设计变更申请编号 -->
      <el-table-column prop="code" width="250" :label="'设计变更申请编号'" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.code}}</template>
      </el-table-column>
      <!-- 设计变更名称 -->
      <el-table-column prop="name" :label="'设计变更名称'" show-overflow-tooltip width="200">
      ...
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="text"
            v-if="(scope.row.approveStatus === '0'|| scope.row.approveStatus === '3') && $checkPersmissionByCode('cost:designAlterationMgmt:create')"
            icon="el-icon-edit"
            :title="$l('btn_edit')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="(scope.row.approveStatus === '0'|| scope.row.approveStatus === '3') && $checkPersmissionByCode('cost:designAlterationMgmt:delete')"
            icon="el-icon-delete"
            :title="$l('btn_delete')"
...
import { getDesignAlterationList, deleteDesignAlteration, updateDalStatus, createDesignAlterationInstruction } from '@/api/cost/designAlteration'
