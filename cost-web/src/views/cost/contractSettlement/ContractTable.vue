...
<template>
  <div ref="ineffectiveCostTable">
    <el-table :data="tableData"
      ...
      <!-- 结算编号 -->
      <el-table-column prop="estimateNo"
                       :label="'结算编号'" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.estimateNo}}
        </template>
      </el-table-column>
      <!-- 合同名称 -->
      ...
      <!-- 结算金额 （元）-->
      <el-table-column prop="estimatePrice"
                       :label="'结算金额（元）'">
        <template slot-scope="scope">
          {{scope.row.estimatePrice | NumFormat}}
        </template>
      </el-table-column>

      <!-- 合同金额 -->
      <el-table-column prop="contractValue"
                       :label="'签约金额（元）'">
        <template slot-scope="scope">
          {{scope.row.contractValue | NumFormat}}
        </template>
      </el-table-column>
      ...
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentContract:createOrUpdate')"
                     icon="el-icon-edit"
                     :title="$l('btn_edit')"
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3')  && $checkPersmissionByCode('cost:paymentContract:delete')"
                     icon="el-icon-delete"
                     :title="$l('btn_delete')"
...
<script>
  ...
  import { getContractList, deleteContract, setFlowStatus } from '@/api/cost/contractAPI'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        // 转换参数
        // alert(JSON.stringify(this.queryObj_passed_in.queryModelTest));
        let _queryObj = {
          ...
          estimateNo: this.queryObj.estimateNo,
