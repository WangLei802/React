        ...
        <template slot-scope="scope">
          <el-row>
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentProject:create')"
              icon="el-icon-edit"
              :title="$l('btn_edit')"
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentProject:delete')"
              icon="el-icon-delete"
              :title="$l('btn_delete')"
...
import { getProjectDataApproveList, deleteProjectDataApprove, setFlowStatus } from '@/api/cost/projectDataApproveAPI'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      getProjectDataApproveList(_queryObj)
        .then(response => {
          //   debugger
    ...
    deleteRow (index, rowData) {
      this.$confirm('确认删除该记录', {
        ...
        deleteProjectDataApprove(rowData)
          .then(response => {
