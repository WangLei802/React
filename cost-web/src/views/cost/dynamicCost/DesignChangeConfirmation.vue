    ...
    <div class='app-container'>
        <div style="text-align:right; margin:10px 0;border-bottom:1px solid #ccc;padding-bottom: 10px;">
             <span class="breadcrumb-child"
                  style="float: left;margin-top: 7px;" v-if="moduleType === '2_1_1'"> 设计变更二次造价确认</span>
            <span class="breadcrumb-child"
                  style="float: left;margin-top: 7px;" v-if="moduleType === '2_1_0'"> 设计变更一次造价确认</span>
            <el-button size="mini"
        ...
        <el-form label-width='150px'
            ...
            <el-row :gutter="12">
                ...
                <el-col :span="8">
                    <!-- 设计变更金额   二次造价 costRetryConfirmAmt-->
                    <el-form-item :label="'设计变更金额(元):'"
                                  prop="costConfirmAmt">
...
export default {
  ...
  methods: {
      ...
      if (this.$route.query.moduleType) {
        ...
        getSplittingAdjust({ 'moduleType': this.$route.query.moduleType, 'moduleDataPK': this.$route.query.moduleDataPk }).then(response => {
          ...
          if (this.form.contractModels_splitting_cp2sb2ct.length > 0) {
            this.activeName = this.form.contractModels_splitting_cp2sb2ct[0].splittingWay
    ...
    saveContractAlteration() {
      LoadingUtil.showLoading()
      // this.form.splittingCp2Sb2Ct = this.form.splittingCp2Sb2Ct[0].splittingModels_cp2sb2ct
