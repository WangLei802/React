    ...
    <el-table :data="tableData"
      ...
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentCashDeposit:createOrUpdate')"
                     icon="el-icon-edit"
                     :title="$l('btn_edit')"
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentCashDeposit:delete')"
                     icon="el-icon-delete"
                     :title="$l('btn_delete')"
...
<script>
  ...
  import { getBidBondList, deleteBidBond , setFlowStatus } from '@/api/cost/bidBondAPI'
  ...
  export default {
    ...
    methods: {
      ...
      viewRow(index, rowData) {
        let returnRateAt = (rowData.cashDepositReturnLower / rowData.cashDepositTotal).toFixed(2)

        this.$router.push({
          name: 'NewOrUpdateBidBond',
          query: { id: rowData.id, isEdit: false,isShow:true }
        })
      },
      newIcon(index,rowData){ 
        ...
        this.$router.push({
          name: 'NewOrUpdateBidBond',
          query: { id: rowData.id, isEdit: false, isShow:false }
        })
      },
      // 编辑
      editRow(index, rowData) {
        ...
        this.$router.push({
          name: 'NewOrUpdateBidBond',
          query: { id: rowData.id, isEdit: true,isShow:true }
        })
      },
