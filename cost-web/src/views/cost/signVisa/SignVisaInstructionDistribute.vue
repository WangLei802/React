<template>
  <div class="app-container">
    <div style="text-align:right; margin:10px 0;">
      ...
      <!--<el-button size="mini"-->
      <!--type="primary"-->
      <!--@click="invalidInstruction"-->
      <!--v-show="isShowInval">作废</el-button>-->
    ...
    <div>
      <el-steps :active="active" :align-center="true" space="70%" style="margin-bottom: 20px">
        <el-step title="现场签证申请" :style="{'cursor':'pointer'}" @click.native="gotoDesign"></el-step>
...
import { updateAlterationStatus } from "@/api/cost/signVisa";
  ...
  methods: {
    ...
    invalidInstruction() {
      this.$confirm("确认作废", {
        ...
        .then(() => {
          let insData = {
            id: this.designAlterationModel.id,
            approveStatus: "4"
          };
          updateAlterationStatus(insData)
            .then(response => {
              this.$showSuccess(response.message);
              this.isShowInval = false;
              this.isShowDistrui = false;
