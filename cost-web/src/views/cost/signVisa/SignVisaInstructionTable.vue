...
<template>
  <div ref="SignVisaInstructionTable">
    <el-table
      :data="tableData"
      stripe
      border
    ...
    >
      ...
      <!-- 签证指令单 -->
      <el-table-column prop="code" show-overflow-tooltip width="250" :label="'签证指令单'">
      ...
      <!-- 所属现场签证号 -->
      <el-table-column prop="alterationVisaCode" show-overflow-tooltip :label="'所属现场签证号'">
      ...
      <el-table-column :label="this.$l('lbl_operation')" width="150">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="text"
            v-if="scope.row.approveStatus === '7' && scope.row.processNode === '4' && scope.row.isRetryCostConfirm === '0' && $checkPersmissionByCode('cost:siteVisaMgmt:costConfirm')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="scope.row.approveStatus === '7' && scope.row.processNode === '3' && $checkPersmissionByCode('cost:siteVisaMgmt:costConfirm')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="scope.row.approveStatus === '2' && scope.row.processNode === '2' && $checkPersmissionByCode('cost:siteVisaMgmt:update')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="scope.row.approveStatus === '3' && scope.row.processNode === '2' && $checkPersmissionByCode('cost:siteVisaMgmt:completionConfirm')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="scope.row.approveStatus === '1' && scope.row.processNode === '2' && $checkPersmissionByCode('cost:siteVisaMgmt:update')"
          ...
          <el-button
            size="mini"
            type="text"
            v-if="(scope.row.approveStatus === '5' || scope.row.approveStatus === '8') && $checkPersmissionByCode('cost:siteVisaMgmt:completionConfirm')"
            icon="el-icon-edit"
            :title="$l('btn_edit')"
...
import { getDesignAlterationInstructionList, updateInstructionStatus } from '@/api/cost/signVisa'
...
export default {
  ...
  methods: {
    ...
    distributeRow (index, rowData) {
      this.$router.push({
        name: 'SignVisaInstructionDistribute',
        query: { id: rowData.id }
      })
    },
    // 回传
    ...
    completionConfirmRow (index, rowData) {
      this.$router.push({
        name: 'SignVisaInstructionCompletionConfirm',
    ...
    costConfirmRow (index, rowData) {
      this.$router.push({
        name: 'SignVisaInstructionCostConfirm',
        query: { id: rowData.id, contractId: rowData.contractId }
      })
    },
    // 造价重新确认
    ...
    viewRow (index, rowData) {
      let redirectPath = {}
      redirectPath.query = { id: rowData.id, isView: true, contractId: rowData.contractId }
