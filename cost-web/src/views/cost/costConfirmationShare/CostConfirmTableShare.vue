  ...
  <div ref="ineffectiveCostTable">
    <el-table :data="tableData"
      ...
      <el-table-column prop="throwTimes"
                       :label="'成就共享分配次数'" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:confirmShare:create')"
                     icon="el-icon-edit"
                     :title="$l('btn_edit')"
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:confirmShare:delete')"
                     icon="el-icon-delete"
                     :title="$l('btn_delete')"
...
<script>
  ...
  import { getCostConfirmListShare, deleteCostConfirmShare, setFlowStatus } from '@/api/cost/costConfirmShare'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        getCostConfirmListShare(_queryObj)
          .then(response => {
            //   debugger
      ...
      viewRow(index, rowData) {
        ...
        // let cashDepositReturnUpper = this.Arabia_to_Chinese(rowData.cashDepositReturnLower)
        this.$router.push({
      ...
      deleteRow(index, rowData) {
        this.$confirm('确认删除该记录', {
          ...
          deleteCostConfirmShare(rowData)
            .then(response => {
