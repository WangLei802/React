  ...
  <div ref="ineffectiveCostTable">
    <el-table :data="tableData"
      ...
      <el-table-column prop="materialName"
                       :label="'材料设备名称'" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.materialName}}
        </template>
      </el-table-column>
      <!-- 涉及工程总价 -->
      <el-table-column prop="engineeringPrices"
                       :label="'涉及工程总价(元)'">
        <template slot-scope="scope">
          {{scope.row.engineeringPrices | NumFormat}}
        </template>
      </el-table-column>
      <!-- 提交日期 -->
      ...
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:budgetQualityPrice:createOrUpdate')"
                     icon="el-icon-edit"
                     :title="$l('btn_edit')"
          ...
          <el-button size="mini"
                     type="text"
                     v-if="(scope.row.status === '0' || scope.row.status === '3')  && $checkPersmissionByCode('cost:budgetQualityPrice:delete')"
                     icon="el-icon-delete"
                     :title="$l('btn_delete')"
...
<script>
  ...
  import { getConstructionCostList, deleteConstructionCost, setFlowStatus } from '@/api/cost/constructionCost'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        // alert("AA"+this.queryObj.contractName)
        let _queryObj = {
        ...
        getConstructionCostList(_queryObj)
          .then(response => {
            //   debugger
      ...
      deleteRow(index, rowData) {
        this.$confirm('确认删除该记录', {
          ...
          deleteConstructionCost(rowData)
            .then(response => {
