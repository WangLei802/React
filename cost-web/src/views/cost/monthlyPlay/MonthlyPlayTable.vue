...
<template>
  <div ref="ineffectiveCostTable">
    ...
    >
      ...
      <!-- 单据编号 -->
      <el-table-column prop="receiptsNo" :label="'单据编号'" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.receiptsNo}}</template>
      </el-table-column>
      <!-- 单据名称 -->
      <el-table-column prop="receiptsName" :label="'单据名称'" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.receiptsName}}</template>
      </el-table-column>

      <!-- 类型 -->
      <el-table-column prop="planType" :label="'类型'">
        <template slot-scope="scope">
          <span v-if="scope.row.planType == 0">月度结算计划</span>
          <span v-else-if="scope.row.planType == 1">年度结算计划</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-row>
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentMonthYearPlan:create')"
              icon="el-icon-edit"
              :title="$l('btn_edit')"
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3') && $checkPersmissionByCode('cost:paymentMonthYearPlan:delete')"
              icon="el-icon-delete"
              :title="$l('btn_delete')"
...
import { getMonthlyPlayList, deleteMonthlyPlay, setFlowStatus } from '@/api/cost/monthlyPlayAPI'
  ...
  methods: {
    ...
    viewRow (index, rowData) {
      this.$router.push({
        ...
        query: {          id: rowData.id, companyId: rowData.companyId, companyName: rowData.companyName, receiptsNo: rowData.receiptsNo, receiptsName: rowData.receiptsName, planType: rowData.planType,
          projectId: rowData.projectId, projectName: rowData.projectName, planContent: rowData.planContent, remark: rowData.remark, operateDate: rowData.operateDate, operator: rowData.operator,
          status: rowData.status, isEdit: false        }
      })
    },
    // 编辑
    editRow (index, rowData) {
      this.$router.push({
        name: 'NewOrUpdateMonthlyPlay',
        query: {          id: rowData.id, companyId: rowData.companyId, companyName: rowData.companyName, receiptsNo: rowData.receiptsNo, receiptsName: rowData.receiptsName, planType: rowData.planType,
          projectId: rowData.projectId, projectName: rowData.projectName, projectCode: rowData.projectCode, planContent: rowData.planContent, remark: rowData.remark, operateDate: rowData.operateDate, operator: rowData.operator,
          status: rowData.status, isEdit: true        }
      })
    },
