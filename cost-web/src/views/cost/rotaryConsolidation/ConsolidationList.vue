      ...
      <mt-searchform>
        <template slot="buttons">
          <el-button icon='el-icon-plus' size="mini"
                      type='primary'  v-if="$checkPersmissionByCode('cost:budgetConveySure:create')"
        ...
        <ConstructionTable :queryObj_passed_in='queryModel'
                          ref="ConstructionTable"></ConstructionTable>
    </div>
</template>
...
import ConstructionTable from '@/views/cost/rotaryConsolidation/ConsolidationTable'
  ...
  methods: {
    updateMessage(item) {
      if (item.type === COSTENUM.projectTreeType.TREE_NODE_ACC_OBJ) {
        ...
        // this.queryModel.projectId = item.projectId
      } else {
