...
<template>
  <div class="app-container">
    ...
    >
        ...
        <template slot-scope="scope">
          <div :style="{'float':'left','text-align':'left','padding-left':scope.row.level*5+'px'}">
            <i
              ...
              @click="handleExpandClick(scope.row.id)"
            ></i>
            {{ scope.row.text}}
          </div>
        </template>
      ...
      <!-- 集团命名 -->
      <el-table-column prop="groupName" label="集团命名" show-overflow-tooltip width="100"></el-table-column>

      <!-- 最新平均售价(含车位) -->
      <el-table-column
        prop="latestAvgSellingPrice"
        label="最新平均售价(含车位)"
        show-overflow-tooltip
        width="100"
      >
        <template slot-scope="scope">{{scope.row.latestAvgSellingPrice | NumFormat}}</template>
      </el-table-column>
      ...
      <el-table-column
        ...
        :key="index"
      >
        <el-table-column width="120" prop="targetCostIncludeTax" label="目标成本(万元)">
          <template
            slot-scope="scope"
          >{{scope.row['stageTargetCostModel'+item.id].targetCostIncludeTax | NumFormat }}</template>
        </el-table-column>

        <el-table-column width="110" prop="constructSmCostIncludeTax" label="建面单方(元/㎡)">
          <template
            slot-scope="scope"
          >{{scope.row['stageTargetCostModel'+item.id].constructSmCostIncludeTax | NumFormat}}</template>
        </el-table-column>

        <el-table-column width="110" prop="saleSmCostIncludeTax" label="可售单方(元/㎡)">
          <template
            slot-scope="scope"
          >{{scope.row['stageTargetCostModel'+item.id].saleSmCostIncludeTax | NumFormat}}</template>
        </el-table-column>

        <el-table-column width="110" prop="saleRatio" label="可售比(%)">
          <template
            slot-scope="scope"
          >{{scope.row['stageTargetCostModel'+item.id].saleRatio | NumFormat}}</template>
        </el-table-column>
      </el-table-column>

      <!-- 动态成本(万元) -->
      <el-table-column width="100" prop="dynamicCost" label="动态成本(万元)">
        <template slot-scope="scope">{{scope.row.dynamicCost | NumFormat}}</template>
      </el-table-column>

      <!-- 预备会召开时间 -->
      <el-table-column width="100" prop="prepareMeetingTime" label="预备会召开时间">
        <template
          slot-scope="scope"
        >{{scope.row.prepareMeetingTime | dateFormat('yyyy-MM-dd HH:mm:ss')}}</template>
      </el-table-column>

      <!-- 预备会-投后版偏差额(万元) -->
      <el-table-column width="100" prop="preMeetingAfterInvestDiff" label="预备会-投后版偏差额(万元)">
        <template slot-scope="scope">{{scope.row.preMeetingAfterInvestDiff | NumFormat}}</template>
      </el-table-column>

      <!-- 预备会-投后版偏差率(%) -->
      <el-table-column width="100" prop="preMeetingAfterInvestDiffRatio" label="预备会-投后版偏差率(%)">
        <template
          slot-scope="scope"
        >{{scope.row.preMeetingAfterInvestDiffRatio===0?'-':scope.row.preMeetingAfterInvestDiffRatio}}</template>
      </el-table-column>

      <!-- 结算版-预结算偏差额(万元) -->
      <el-table-column width="100" prop="settlementPreSettlementDiff" label="结算版-预结算偏差额(万元)">
        <template slot-scope="scope">{{scope.row.settlementPreSettlementDiff | NumFormat}}</template>
      </el-table-column>

      <!-- 结算版-预结算偏差率(%) -->
      <el-table-column width="100" prop="settlementPreSettlementDiffRatio" label="结算版-预结算偏差率(%)">
        <template
          slot-scope="scope"
        >{{scope.row.settlementPreSettlementDiffRatio===0?'-':scope.row.settlementPreSettlementDiffRatio}}</template>
      </el-table-column>

      <!-- 备注(成本预警) -->
      <el-table-column width="100" prop="remaker" label="备注(成本预警)">
        <template slot-scope="scope">
          <i
            class="fa fa-circle"
            :style="{'color':scope.row.remaker>1?(scope.row.remaker>1.02?'red':'orange'):'lightgreen'}"
          ></i>
        </template>
      </el-table-column>

      <!-- 建筑面积 -->
      <el-table-column width="100" prop="totalBuildingArea" label="建筑面积">
        <template slot-scope="scope">{{scope.row.totalBuildingArea | NumFormat}}</template>
      </el-table-column>

      <!-- 可售面积 -->
      <el-table-column width="100" prop="totalSellArea" label="可售面积">
        <template slot-scope="scope">{{scope.row.totalSellArea | NumFormat}}</template>
      </el-table-column>

      <!-- 全地库面积 -->
      <el-table-column width="100" prop="allBasementArea" label="全地库面积">
        <template slot-scope="scope">{{scope.row.allBasementArea | NumFormat}}</template>
      </el-table-column>

      <!-- 地下停车位 -->
      <el-table-column width="100" prop="undergroundParking" label="地下停车位">
        <template slot-scope="scope">{{scope.row.undergroundParking | NumFormat}}</template>
      </el-table-column>

      <!-- 单车位平均面积 -->
      <el-table-column width="100" prop="parkingAvgArea" label="单车位平均面积">
        <template slot-scope="scope">{{scope.row.parkingAvgArea | NumFormat}}</template>
      </el-table-column>

      <!-- 主要业态 -->
      <el-table-column width="100" prop="majorBusinessType" label="主要业态">
        <template slot-scope="scope">{{scope.row.majorBusinessType}}</template>
      </el-table-column>
    </el-table>
  </div>
</template>
