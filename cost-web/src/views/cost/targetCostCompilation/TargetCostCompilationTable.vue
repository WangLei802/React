  ...
  <div ref="targetCostCompilationTable">
    ...
    >
      ...
      <!-- <el-table-column prop="stageVersion"
                             ...
                             width="160">
                <template slot-scope="scope">
                    {{scope.row.stageVersion === '0'?'成本编制':'成本调整'}}
                </template>
      </el-table-column>-->
      <!-- 阶段 -->
      ...
      <!-- 目标成本总额(含税) -->
      <el-table-column prop="targetCost" :label="'目标成本总额(含税)(元)'" width="180">
        <template slot-scope="scope">{{ scope.row.targetCost|NumFormat}}</template>
      </el-table-column>
      <!-- 版本 -->
      <el-table-column
        prop="stageVersion"
        :render-header="openAnnotation"
        label-class-name="每编制一个阶段的目标成本为一个大的版本，如投前V0，投后V0，预备会V0，预结算V0，结算V0。每调整一次目标成本为一个小的版本，如“投后V0.1”表示投后第一次总金额不突破的调整，“预备会V13.0”表示预备会第13次总金额突破的调整。"
        :label="'版本'"
      >
      ...
      <el-table-column prop="status" :label="this.$l('lbl_status')">
        <template slot-scope="scope">{{selectData.cost_flow_status[scope.row.status]}}</template>
      </el-table-column>
      <!-- 操作 -->
      <el-table-column :label="this.$l('lbl_operation')">
        <template slot-scope="scope">
          <el-row>
            ...
            <el-button
              size="mini"
              v-if="scope.row.status === '2' && !scope.row._showExpanded  && $checkPersmissionByCode('cost:targetCostMgmt:save')"
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3')  && $checkPersmissionByCode('cost:targetCostMgmt:saveOrUpdate')"
              icon="el-icon-edit"
              :title="$l('btn_edit')"
            ...
            <el-button
              size="mini"
              type="text"
              v-if="(scope.row.status === '0' || scope.row.status === '3') && scope.row.autoCreate!=='1' && $checkPersmissionByCode('cost:targetCostMgmt:delete') "
              icon="el-icon-delete"
              :title="$l('btn_delete')"
...
import {
  listAllTargetCostCompilation,
  deleteTargetCostCompilation,
  getTargetCosVersiontList,
  setFlowStatus
  ...
  methods: {
    ...
    initTableData (response) {
      ...
      filterData.map(item => {
        ...
        item._showExpanded = !!(_saveAccountingObject && item.accountingObjectName !== '')
      })
          ...
          if (!response.message && response.code === '2000') {
            ...
            this.$router.push({
              ...
              query: {
                isAdjust: true, // 是否调整
                accountingObjectId: rowData.accountingObjectId,
                accountingObjectText: rowData.accountingObjectText,
    ...
    viewRow (index, rowData) {
      // this.$router.push({ name: 'CostTest' })
      // return
      this.$router.push({
        name: rowData.stageVersion === '0' ? 'NewOrUpdateTargetCostCompilation' : 'NewOrUpdateTargetCostAdjustment',
        query: {
          accountingObjectId: rowData.accountingObjectId,
          accountingObjectText: rowData.accountingObjectText,
          stageId: rowData.stageId,
    ...
    deleteRow (index, rowData) {
      this.$confirm('确认删除该记录', {
        ...
        LoadingUtil.showLoading()
        deleteTargetCostCompilation(rowData)
          .then(response => {
    ...
    openAnnotation: function (h, { column, $index }) {
      return (
        <el-tooltip effect='dark' placement='bottom'>
          ...
          <span title='每编制一个阶段的目标成本为一个大的版本，如投前V0，投后V0，预备会V0，预结算V0，结算V0。每调整一次目标成本为一个小的版本，如“投后V0.1”表示投后第一次总金额不突破的调整，“预备会V13.0”表示预备会第13次总金额突破的调整。'>{column.label} <i class='el-icon-info'></i></span>
        </el-tooltip>
      )
    }
  },
