# **React 采坑记 * 基础篇**
本人前端码畜一枚 跟大家分享下自己采坑记  记录个人对react学习的一点理解与分享  希望可以跟大家共同学习，共同进步，共同探讨    大家多多指教  不喜勿喷  (希望可以对react有共同兴趣的小伙伴有帮助)
...
#### **切记不要直接修改 State  (此处修改数据的方式有点类似于小程序的写法)**
...
子组件Name.propTypes={
    参数名:PropTypes.参数类型,
    参数名:PropTypes.参数类型.isRequired     //此项说明该参数必须传递，若没有则会报错
}
```
### 也可给定默认值
```
子组件Nmae.defaultProps = {
    参数名:键值                         //父组件不进行传递也会存在默认值
}
```
## **下一步学习React的生命周期**
